                            ...
                            <main>
                                <div class="container-fluid px-4">
                                    ...
                                    <ol class="breadcrumb mb-4">
                                        <li class="breadcrumb-item"><a href="<c:url value='/admin' />">Dashboard</a>
                                        </li>
                                        <li class="breadcrumb-item"><a href="<c:url value='/admin/order' />">Order</a>
                                        </li>
                                                ...
                                                <form:form method="post" action="/admin/order/update" class="row"
                                                    ...
                                                    <div class="mb-3 col-12 col-md-6">
                                                        ...
                                                        <form:select class="form-select" path="status" 
                                                                     disabled="${newOrder.status == 'RETURNED'}">
                                                            <c:forEach var="status" items="${orderStatuses}">
                                                                <form:option value="${status}">${status}</form:option>
                                                            </c:forEach>
                                                        </form:select>
                                                        <c:if test="${newOrder.status == 'RETURNED'}">
                                                            <small class="text-danger">
                                                                ⚠️ Đơn hàng đã hoàn trả không thể thay đổi trạng thái
                                                            </small>
                                                        </c:if>
                                                    </div>
                                                    <div class="col-12 mb-5">
                                                        <button type="submit" class="btn btn-warning" 
                                                                ${newOrder.status == 'RETURNED' ? 'disabled' : ''}>
                                                            Update
                                                        </button>
                                                    </div>
                                                </form:form>
