(function ($) {
  ...
  $(".quantity button").on("click", function () {
    ...
    // Get maximum available quantity from product
    const maxQuantity = input.attr("data-product-quantity");

    if (button.hasClass("btn-plus")) {
      var newVal = parseFloat(oldValue) + 1;

      // Check if new value exceeds maximum available quantity
      if (maxQuantity && newVal > parseFloat(maxQuantity)) {
        // Show notification that maximum quantity reached
        if (typeof $.toast === "function") {
          $.toast({
            heading: "Giới hạn số lượng",
